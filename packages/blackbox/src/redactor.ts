...
export class Redactor {
  ...
  /** 对文本执行脱敏 */
  redactText(text: string): { result: string; matches: { rule: string; count: number }[] } {
    let result = text;
    const matches: { rule: string; count: number }[] = [];
  ...
  /** 对对象递归脱敏 */
  redactObject<T>(obj: T): { result: T; matches: { rule: string; count: number }[] } {
    const allMatches: { rule: string; count: number }[] = [];
    const result = this._redactDeep(obj, allMatches) as T;
    return { result, matches: allMatches };
  }

  private _redactDeep(obj: unknown, allMatches: { rule: string; count: number }[]): unknown {
    ...
    if (obj !== null && typeof obj === 'object') {
      ...
      for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
        // 跳过签名字段
        if (key === 'signature') { out[key] = value; continue; }
